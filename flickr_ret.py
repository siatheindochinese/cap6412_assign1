...
# Load Dataset #
################
YOUR_LOCAL_PATH = 'cache/flickr30k'
coco_test = load_dataset(YOUR_LOCAL_PATH, data_files={'test':'annotations/test.json'})['test']
...
if 'itc.txt' not in os.listdir('precomputed_flickr'):
	...
	np.savetxt('precomputed_flickr/itc.txt', itc)
else:
	itc = np.loadtxt('precomputed_flickr/itc.txt')
	
# filter top k=128 for i2t
# i2t, matrix of n(i)x128
# elements are the top k=128 text indices
if 'i2t_idx.txt' not in os.listdir('precomputed_flickr'):
	i2t_idx = np.argpartition(itc, -k)[:,-k:]
	np.savetxt('precomputed_flickr/i2t_idx.txt', i2t_idx)
else:
	i2t_idx = np.loadtxt('precomputed_flickr/i2t_idx.txt').astype(int)
	
# filter top k=128 for t2i 
# i2t, matrix of n(t)x128
# elements are the top k=128 image indices
if 't2i_idx.txt' not in os.listdir('precomputed_flickr'):
	t2i_idx = np.argpartition(itc.T, -k)[:,-k:]
	np.savetxt('precomputed_flickr/t2i_idx.txt', t2i_idx)
else:
	t2i_idx = np.loadtxt('precomputed_flickr/t2i_idx.txt').astype(int)
...
if 'itm_i2t.txt' not in os.listdir('precomputed_flickr'):
	for i in range(len(image)):
		for j in range(k):
			print(i,j)
			t  = i2t_idx[i][j]
	...
	np.savetxt('precomputed_flickr/itm_i2t.txt', itm_i2t)
else:
	itm_i2t = np.loadtxt('precomputed_flickr/itm_i2t.txt')

# ITM score for t2i
itm_t2i = np.zeros((len(text), k))
if 'itm_t2i.txt' not in os.listdir('precomputed_flickr'):
	for i in range(len(text)):
		for j in range(k):
			print(i,j)
			t  = t2i_idx[i][j]
	...
	np.savetxt('precomputed_flickr/itm_t2i.txt', itm_t2i)
else:
	itm_t2i = np.loadtxt('precomputed_flickr/itm_t2i.txt')
